...
/**
 * @brief the function used to do multicore texture loading
 * 
 * @param params parameters for function
*/

/**
 * @brief the function used to do multicore drawing
 * 
 * @param params parameters for function
*/
void xDrawTexture(void* params)
{
  uint8_t* selected_texture = (uint8_t*)((uint8_t**)params)[1];
  uint16_t x = ((uint16_t*)params)[2];
  uint16_t y = ((uint16_t*)params)[3];
  uint16_t pointer_pos = (y*(DISPLAY_W/8))+((x-(x%8))/8);
  for(uint16_t i = 0; i < ((uint16_t*)params)[4]; i++)
  {
    memcpy((uint8_t*)((uint8_t*)((uint8_t**)params)[2]+pointer_pos), (uint8_t*)(selected_texture+(i*((uint16_t*)params)[5]/8)), (((uint16_t*)params)[5]/8));
    pointer_pos+=(DISPLAY_W/8);
  }
}

/**
 * @brief calls a gl function that gets attached to diffrent core
 * 
 * @param func function to be called
 * 
 * @param fr first parameter
 * 
 * @param se second parameter
 * 
 * @param th third parameter
*/
...
{
  TaskHandle_t* handle;
  uint32_t size = 2*sizeof(uint8_t*) + 4*sizeof(uint16_t);
  void* params = (void*)malloc(size);
  ((uint8_t**)params)[0] = contextBuffer;
  ((uint8_t**)params)[1] = texture;
  ((uint16_t*)params)[2] = x;
  ((uint16_t*)params)[3] = y;
  ((uint16_t*)params)[4] = tex_w;
  ((uint16_t*)params)[5] = tex_h;
